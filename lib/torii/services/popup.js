...
function validateToriiMessage(message){
  return message && message.indexOf(CURRENT_REQUEST_KEY) === 0;
}

function parseWindowMessage(message, keys){
  var url = message.slice(CURRENT_REQUEST_KEY.length),
      parser = new ParseQueryString({url: url, keys: keys}),
      data = parser.parse();
  return data;
}
    ...
    return new Ember.RSVP.Promise(function(resolve, reject){
      ...
      Ember.$(window).on('message.torii', function(event){
        ...
        if (validateToriiMessage(message)) {
          var data = parseWindowMessage(message, keys);
          Ember.run(function() {
            resolve(data);
          });
        }
